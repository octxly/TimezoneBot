...
const cmd = {
    name: "setuser",
    desc: "Sets the specified timezone of any user",
    arguements: [
        {
            type: "USER",
            name: "user",
            description: "The user the timezone role is being added to.",
            required: true
        },
        {
            type: "INTEGER",
            name: "hours",
            description: "The hours of the UTC offset.",
            required: true,
            minValue: -12,
            maxValue: 14
        },
        {
            type: "INTEGER",
            name: "minutes",
            description: "The minutes of the UTC offset.",
            required: false,
            choices: [
                { name: "15", value: 15 },
                { name: "30", value: 30 },
    ...
    async execute(params) {
        ...
        const mentioned = options.getMember("user");
        ...
        if (!(await GuildModel_1.GuildModel.exists({ guildId: guild.id }))) {
            await new GuildModel_1.GuildModel({
                guildId: guild.id,
                timezones: []
            }).save();
        }
